...
发布账号若启用了 **npm 双因素认证（2FA）**，本地 `npm publish` 时需按提示完成 OTP；CI 请使用 **Automation** 类型 token（见下），并在 `publish.yml` 中配置 `NPM_TOKEN`。
...
> 发布前请确认 `package.json` 的 **`version` 已递增**（`npm version patch/minor/major`），且该版本号未被 npm 占用；tag 与 version 建议保持一致（如 `v0.1.1` ↔ `0.1.1`）。
